...
bool GTNetwork::create_graph() {
    ...
    for (const auto& [id, cs] : this->c_ss) {
        auto neighbours_pipes = cs.get_links()[1]; // ????????? ?????
...
bool GTNetwork::add_node() {
    ...
    // ?????? ??????? ?????
    cout << "output id: ";
    int output_id = GetCorrectNumber<int, std::unordered_map<int, CS>>("output id: ", this->c_ss, IsExistingObj);

    // ???? ??????? ?????
    cout << "input id: ";
    int input_id = GetCorrectNumber<int, std::unordered_map<int, CS>>("input id: ", this->c_ss, IsExistingObj);
...
bool GTNetwork::delCSFromGraph() {
    ...
    while (true) {
        std::cout << "input id(0 for exit): ";
        id = GetCorrectNumber<int, std::unordered_map<int, CS>>("input correct id: ", this->c_ss, ZeroRange);
